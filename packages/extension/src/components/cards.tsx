...
function CopyButton({ text, label }: { text: string; label: string }) {
	...
	return (
		<button
			...
			className="text-[9px] text-muted-foreground hover:text-foreground border px-1 rounded shrink-0 cursor-pointer"
		>
			{copied ? 'Copied!' : label}
		</button>
	)
			...
			<div className="flex items-center gap-3 -my-1">
				{rawRequest != null && (
					<button
						...
						className={cn(
							'text-[10px] mt-0.5 border-b cursor-pointer',
							activeTab === 'request'
...
export function ActivityCard({ activity }: { activity: AgentActivity }) {
	...
	return (
		<div className="flex items-center gap-2 rounded-lg border bg-muted/40 p-2.5">
			<div className={cn('size-2 rounded-full', info.color.replace('text-', 'bg-'))} />
